...
async function stylistDisplayName(salonId, stylistId, gender = "any") {
  if (!stylistId || stylistId === "0" || stylistId === "none") return "No Preference";
  const list = await getStylistsByGender(salonId, gender);
  const match = list.find((s) => s.id === stylistId);
  return match ? match.name : "No Preference";
}
...
function filterSlotsFromCurrentTime(slots, bookingDate) {
  ...
  const now = getZonedNowParts();
  const cutoffMinutes = Number(now.hour) * 60 + Number(now.minute) + 90;
...
async function buildReviewScreenData(incoming, session) {
  ...
  const noStylist = !d.stylist_id || d.stylist_id === "0" || d.stylist_id === "none" || d.stylist_id === "-";
  const stylist_name = noStylist
    ? "-"
  ...
  row.review_summary = [
    ...
    `Date: ${(() => { const m = String(row.booking_date || "").match(/^(\d{4})-(\d{2})-(\d{2})$/); return m ? `${m[3]}/${m[2]}/${m[1]}` : (row.booking_date || "—"); })()}`,
    `Time: ${row.slot_id}`,
...
export async function handleFlowDataExchange(reqBody) {
  ...
  if (screen === "DATE_STYLIST" && act === "data_exchange") {
    ...
    if (isPastDate(merged.booking_date)) {
      ...
      const reloadedStylists = await getStylistsByGender(merged.salon_id, merged.gender, minDate);
      return {
        ...v,
        screen: "DATE_STYLIST",
        data: {
          ...
          stylist_options: [
            { id: "0", title: "No Preference" },
            ...reloadedStylists.map((s) => ({ id: s.id, title: s.displayTitle || s.name }))
          ]
        }
      };
    }
    ...
    return {
      ...
      data: {
        ...
        stylist_id: (!nextSession.stylist_id || nextSession.stylist_id === "0" || nextSession.stylist_id === "none") ? "-" : nextSession.stylist_id,
        stylist_name: (!nextSession.stylist_id || nextSession.stylist_id === "0" || nextSession.stylist_id === "none") ? "-" : stylist_name,
  ...
  if (screen === "REVIEW" && act === "complete") {
    ...
    try {
      const addToCalendarPayload = {
        ...
        id: (!d.stylist_id || d.stylist_id === "0" || d.stylist_id === "none" || d.stylist_id === "-") ? "" : String(d.stylist_id || "")
      };
