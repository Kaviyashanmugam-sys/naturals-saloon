...
app.get("/internal/debug-report", async (_req, res) => {
  ...
  try {
    ...
    steps.push(`key_present=${!!apiKey} length=${apiKey?.length || 0}`);

    steps.push("calling getDailyStats");
    const IST_OFFSET_MS = (5 * 60 + 30) * 60 * 1000;
    const today = new Date(Date.now() + IST_OFFSET_MS).toISOString().slice(0, 10);
    const stats = await getDailyStats(today);
    steps.push(`stats OK total=${stats.total}`);

    steps.push("calling PDFShift directly");
    ...
    steps.push(`pdfshift status=${pdfRes.status}`);

    if (!pdfRes.ok) {
      const errText = await pdfRes.text();
      steps.push(`pdfshift error body=${errText.slice(0, 500)}`);
      return res.json({ ok: false, steps });
    }

    const buf = Buffer.from(await pdfRes.arrayBuffer());
    steps.push(`pdf generated bytes=${buf.length}`);
    return res.json({ ok: true, steps });
  } catch (e) {
    steps.push(`EXCEPTION: ${e.message}`);
    return res.json({ ok: false, steps, errorStack: e.stack });
  }
});
